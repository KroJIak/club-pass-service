...
async def safe_edit_message(
    ...
    try:
        ...
        if new_message:
            # New message was sent (photo was added), so delete old system message if different
            if old_system and not (old_chat_id == new_message.chat.id and old_message_id == new_message.message_id):
                await temporary_messages_middleware.delete_system_message(bot, old_chat_id, old_message_id)
            await _after_system_action(bot, user_id, new_message.chat.id, new_message.message_id)
        else:
            # Message was edited in place - delete old system message if it's different from the one being edited
            if old_system and not (old_chat_id == chat_id and old_message_id == callback.message.message_id):
                await temporary_messages_middleware.delete_system_message(bot, old_chat_id, old_message_id)
